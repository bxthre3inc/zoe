...
### Phase 5: Work Pattern Detection

Search for emerging patterns in how the user works:

```bash
# Recent decisions and preferences
python3 /home/workspace/Skills/supermemory/scripts/memory.py search \
  --query "decided prefers pattern" --limit 20

# User profile for baseline
python3 /home/workspace/Skills/supermemory/scripts/memory.py profile
```

Look for patterns in:
- **Decision style**: Research-heavy vs intuitive, consensus-seeking vs autonomous, reversible vs permanent
- **Work rhythms**: Sprint-and-rest vs steady, morning vs evening focus, meeting tolerance
- **Communication**: Detail level preferences, async vs sync, notification thresholds
- **System design**: Control vs emergence, explicit boundaries vs organic flow, canonical vs flexible

Compare against previously saved patterns. Save new observations:

```bash
python3 /home/workspace/Skills/supermemory/scripts/memory.py save \
  --content "Pattern observation: [specific behavior] in [context]. Previous pattern was [X], now seeing [Y]." \
  --tags "work-pattern,preference,[category]"
```
...
```
send_sms_to_user(
  message="Weekly self-audit done. [X] skills healthy, [Y] patterns detected. Top: [brief pattern or proposal]. Full report in Records/Reflections/."
)
```
