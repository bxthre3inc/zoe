...
- **District Hubs (DHU) [Level 2 - Regional Mesh Manager]:** Edge coordinators mounted on 35-foot Class 4 timber poles. Line-of-sight 10km radius. Runs an **NVIDIA Jetson Nano** for Maxwell-GPU accelerated spatial processing. Capable of instant "Reflex Logic" responses bypassing cellular latency. Processes **20m and 10m spatial fidelity** grids.
  - **Audit Node Addendum**: Houses a cryptographically signed (128-bit AES) "Black Box" cache.

## Field-Level Edge Hardware

Sensors and actuators deployed below and above ground across field Zones. Subsurface equipment (LRZ/VFA) utilizes **Schedule 80 Impact-Modified PVC** for maximum chemical resistance and structural rigidity, while above-ground infrastructure requires UV-shielded (fluoropolymer coated) Polycarbonate and Hybrid Pulse Capacitors (HPC) for extreme-cold -30°F survivability.

- **Vertical Field Anchors (VFA) [Level 1 - Advanced Peer Node]:** The 48-inch deep-profile central ground-truth soil node. Utilizes **Schedule 80 Impact-Modified PVC** for structural rigidity in high-alkali soil. Downgraded from a routing hub to a highly efficient peer, it uses a flush 3-foot antenna to transmit 128-bit encrypted FHSS payload data directly to the elevated PMT.
...
- **Pivot Motion Trackers (PMT) [Level 1.5 - Field Hub & Edge-EBK Engine]:** The command center and Nervous System of the field, mounted 10-15 feet high on the pivot span.
  - **RF Umbrella:** Receives **900MHz FHSS AES-128 chirps** from the VFA, LRZs, and PFA, bundling the entire field state into a single ~187-byte AES-256 payload and blasting it to the DHU via the best available backhaul (e.g., 5GHz LTU, LoRaWAN, Cellular).
