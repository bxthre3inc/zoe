...
Sensors and actuators deployed below and above ground across field Zones. Subsurface equipment (LRZ/VFA) utilizes **HDPE SDR9 or 11** for maximum chemical resistance and structural rigidity, while above-ground infrastructure requires UV-shielded (fluoropolymer coated) Polycarbonate and Hybrid Pulse Capacitors (HPC) for extreme-cold -30°F survivability.

- **Vertical Field Anchors (VFA) [Level 1 - Advanced Peer Node]:** The 48-inch deep-profile central ground-truth soil node. Utilizes **HDPE SDR9 or 11** for maximum chemical resistance and impact strength in high-alkali soil. Communicates via 128-bit encrypted 900MHz FHSS directly to the elevated PMT.
...
- **Pivot Motion Trackers (PMT) [Level 1.5 - Field Hub & Edge-EBK Engine]:** The command center and Nervous System of the field, mounted 10-15 feet high on the pivot span.
  - **RF Umbrella:** Receives **900MHz FHSS AES-128 chirps** from the VFA, LRZs, and PFA, bundling the entire field state into a single ~187-byte AES-256 payload and beaming it to the DHU via high-speed **2.4GHz / 5GHz wireless (WiFi-style bridge)**.
